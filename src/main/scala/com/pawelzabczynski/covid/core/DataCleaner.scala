  ...
  implicit class CleanerImplicits(val df: DataFrame) {
    ...
    def cleanNA[A <: Product]()(implicit tag: TypeTag[A]): Dataset[A] = {
      ...
      val resultDf = colNameToDefaultValue.foldLeft(toCleanDF) {
        case (acc, rm) if rm.dataType == StringType =>
          acc.fillNull(rm.dataType, rm.colName).na.replace(rm.colName, StringTypeToDefaultValue)
        case (acc, rm) if rm.dataType == LongType => acc.fillNull(rm.dataType, rm.colName).na.replace(rm.colName, LongTypeToDefaultValue)
        case (acc, rm) if rm.dataType == IntegerType =>
          acc.fillNull(rm.dataType, rm.colName).na.replace(rm.colName, IntegerTypeToDefaultValue)
        case (acc, rm) if rm.dataType == DoubleType =>
          acc.fillNull(rm.dataType, rm.colName).na.replace(rm.colName, DoubleTypeToDefaultValue)
        case (acc, rm) if rm.dataType == BooleanType =>
          acc.fillNull(rm.dataType, rm.colName).na.replace(rm.colName, BooleanTypeToDefaultValue)
        case (acc, rm) if rm.dataType == TimestampType =>
          acc.fillNull(rm.dataType, rm.colName).na.replace(rm.colName, TimestampTypeToDefaultValue)
